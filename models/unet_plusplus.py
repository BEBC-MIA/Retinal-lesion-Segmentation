...
def res_block(inputs, channel_size, kernel_size, step=None, normalize='gn', down_sample=False, group=32):
    if down_sample:
        ...
        r1 = inputs
    if normalize == 'gn':
        x = GroupNormalization(name='gn_' + step + '0', group=group)(inputs)
    else:
        x = BatchNormalization(name='bn_' + step + '0')(inputs)
    x = relu(x)
    x = conv_bn(x, channel_size, kernel_size, step=step + '1', active=False, normalize=normalize, group=group)
    x = relu(x)
    x = conv_bn(x, channel_size, kernel_size, step=step + '2', active=False, normalize=None)
    x = add([x,r1],name='add_1_'+step)
    r2 = x
    if normalize == 'gn':
        x = GroupNormalization(name='gn_' + step + '2', group=group)(x)
    else:
        x = BatchNormalization(name='bn_' + step + '2')(x)
    x = relu(x)
    x = conv_bn(x, channel_size, kernel_size, step=step + '3', active=False, normalize=normalize, group=group)
    x = relu(x)
    x = conv_bn(x, channel_size, kernel_size, step=step + '4', active=False, normalize=None)
    x = add([x, r2], name='add_2_' + step)
    r3 = x
    if normalize == 'gn':
        x = GroupNormalization(name='gn_' + step + '4', group=group)(x)
    else:
        x = BatchNormalization(name='bn_' + step + '4')(x)
    x = relu(x)
    x = conv_bn(x, channel_size, kernel_size, step=step + '5', active=False, normalize=normalize, group=group)
    x = relu(x)
    x = conv_bn(x, channel_size, kernel_size, step=step + '6', active=False, normalize=None)
    x = add([x, r3], name='add_3_' + step)
    if normalize == 'gn':
        x_decoder = GroupNormalization(name='gn_' + step + '6', group=group)(x)
    else:
        x_decoder = BatchNormalization(name='bn_' + step + '6')(x)
...
'''
'''
#original resblock x3
def res_block(inputs, channel_size, kernel_size, step=None, normalize='gn', down_sample=False, scse=False):
    if down_sample:
        r1 = conv_bn(inputs, channel_size, 1, step=step + 'res', active=False, normalize=normalize)
    else:
        r1 = inputs   
    x1 = conv_bn(inputs, channel_size, kernel_size, step=step + '1', active=True, normalize=normalize)
    x1 = conv_bn(x1, channel_size, kernel_size, step=step + '2', active=False, normalize=normalize)
    x1 = add([x1,r1],name='add_1_'+step)
    x1 = relu(x1)
    r2 = x1
    x2 = conv_bn(x1, channel_size, kernel_size, step=step + '3', active=True, normalize=normalize)
    x2 = conv_bn(x2, channel_size, kernel_size, step=step + '4', active=False, normalize=normalize)
    x2 = add([x2, r2], name='add_2_' + step)  
    x2 = relu(x2)
    r3 = x2
    x3 = conv_bn(x2, channel_size, kernel_size, step=step + '5', active=True, normalize=normalize)
    x3 = conv_bn(x3, channel_size, kernel_size, step=step + '6', active=False, normalize=normalize)  
    x3 = add([x3, r3], name='add_3_' + step)
    x3 = relu(x3)
...
def res_block(inputs, channel_size, kernel_size, step=None, normalize='gn', down_sample=False, scse=False):
    ...
    x3 = relu6(x3)
    r4 = x3
    x4= conv_bn(x3, channel_size, kernel_size, step=step + '7', active=True, normalize=normalize)
    x4 = conv_bn(x4, channel_size, kernel_size, step=step + '8', active=False, normalize=normalize)  
    x4 = add([x4, r4], name='add_4_' + step)
    x4 = relu6(x4)
    if scse == True:
        x = scSE_block(x, step)
...
def WF_block(inputs,channel_size,normalize = 'bn'):
    wf=[]
    for i in range(channel_size):
        x = Conv2D(1,1,padding = 'same',name='wf_conv_'+str(i), kernel_initializer = 'he_normal')(inputs)
        x = Activation('relu')(x)
        wf.append(x)
    x = concatenate(wf,axis = 3,name = 'wf_conc_')
    return x


def fpn(inputs,input_size,channel_list=None,normalize='bn'):
    if channel_list==None:
        channel_list=[32,64,128,256]
    #fpn1 = MaxPooling2D(pool_size=(2, 2),name='fpn1_maxpool_1')(inputs)
    fpn1 = Lambda(lambda image: tf.compat.v1.image.resize(image, (input_size[0]//2, input_size[1]//2),method = 0))(inputs) 
    #fpn2 = MaxPooling2D(pool_size=(2, 2),name='fpn1_maxpool_2')(fpn1)
    fpn2 = Lambda(lambda image: tf.compat.v1.image.resize(image, (input_size[0]//4, input_size[1]//4),method = 0))(fpn1) 
    #fpn3 = MaxPooling2D(pool_size=(2, 2),name='fpn1_maxpool_3')(fpn2)
    fpn3 = Lambda(lambda image: tf.compat.v1.image.resize(image, (input_size[0]//8, input_size[1]//8),method = 0))(fpn2)
    #fpn4 = MaxPooling2D(pool_size=(2, 2),name='fpn1_maxpool_4')(fpn3)
    fpn4 = Lambda(lambda image: tf.compat.v1.image.resize(image, (input_size[0]//16, input_size[1]//16),method = 0))(fpn3)
    '''
    fpn0 = Conv2D(channel_list[0], 1,strides=1,padding = 'same',name='fpn_conv_0', kernel_initializer = 'he_normal')(inputs)    
    if normalize == 'bn':
        fpn0 = BatchNormalization(momentum=0.99,name='fpn_bn_0')(fpn0) 
    elif normalize == 'gn':
        fpn0 = GroupNormalization(name='fpn_gn_0')(fpn0) 
    fpn0 = Activation('relu', name='fpnact_0')(fpn0)   
    ''' 
    fpn1 = Conv2D(channel_list[0], 1,strides=1,padding = 'same',name='fpn_conv_1', kernel_initializer = 'he_normal')(fpn1)
    if normalize == 'bn':
        fpn1 = BatchNormalization(momentum=0.99,name='fpn_bn_1')(fpn1)
    elif normalize == 'gn':
        fpn1 = GroupNormalization(name='fpn_gn_1')(fpn1) 
    fpn1 = Activation('relu')(fpn1)
       
    fpn2 = Conv2D(channel_list[1], 1,strides=1,padding = 'same',name='fpn_conv_2', kernel_initializer = 'he_normal')(fpn2)
    if normalize == 'bn':
        fpn2 = BatchNormalization(momentum=0.99,name='fpn_bn_2')(fpn2)
    elif normalize == 'gn':
        fpn2 = GroupNormalization(name='fpn_gn_2')(fpn2) 
    fpn2 = Activation('relu')(fpn2)  
    
    fpn3 = Conv2D(channel_list[2], 1,strides=1,padding = 'same',name='fpn_conv_3', kernel_initializer = 'he_normal')(fpn3)
    if normalize == 'bn':
        fpn3 = BatchNormalization(momentum=0.99,name='fpn_bn_3')(fpn3)
    elif normalize == 'gn':
        fpn3 = GroupNormalization(name='fpn_gn_3')(fpn3) 
    fpn3 = Activation('relu')(fpn3)   
    
    fpn4 = Conv2D(channel_list[3], 1,strides=1,padding = 'same',name='fpn_conv_4', kernel_initializer = 'he_normal')(fpn4)
    if normalize == 'bn':
        fpn4 = BatchNormalization(momentum=0.99,name='fpn_bn_4')(fpn4)
    elif normalize == 'gn':
        fpn4 = GroupNormalization(name='fpn_gn_4')(fpn4) 
    fpn4 = Activation('relu')(fpn4)       
    #return fpn0,fpn1,fpn2,fpn3,fpn4
...
def fpnv2(inputs,normalize = 'gn'):    
    ...
    fpn3 = conv_bn(fpn3,64,1,step = 'fpn2',active = True,normalize = normalize)
    ...
    return fpn1,fpn2,fpn3,fpn4
def dense_unet_plusplus(pretrained_weights=None,
    ...
    if use_ps == True:
        ...
        conv00 = conv_bn(x,48,3,step='input_conv',active=True,normalize=normalize,group=24)
        block_00 = dense_block(conv00, 48, 3, step='00', normalize=normalize,r=3)
    else:
        ...
        conv00 = conv_bn(x, 32, 3, step='input_conv', active=False, normalize=None)
    ...
    block_10 = dense_block(pool_10, 48, 3, step='10', normalize=normalize,r=6)
    ...
    block_20 = dense_block(pool_20, 48, 3, step='20', normalize=normalize,r=8)
    block_20 = Dropout(0.2, name='dp_0')(block_20)
    ...
    block_30 = dense_block(pool_30, 48, 3, step='30', normalize=normalize,r=8)
    block_30 = Dropout(0.2,name='dp_1')(block_30)
    ...
    block_40 = dense_block(pool_40, 48, 3, step='40', normalize=normalize,r=8)
    ...
    print(model.summary())
    return model
def at_unet_plusplus(pretrained_weights=None,
                  input_size=(512, 512, 1),
                  classes=3,
                  activation='softmax',
                  supervision=True,
    ...
    if use_ps == True:
        w, h = w // 2, h // 2
        x = DeSubpixelConv2D(downsampling_factor=2)(inputs)
        block_00 = standard_block(x, 64, 3, step='00x', normalize=normalize)
    else:
        # inputs = conv_bn(inputs,12,1,step = 'input',active = True,normalize = 'gn',group=12)  #调整通道数和Subpixel后一样
        w, h = w // 2, h // 2
        x = Lambda(lambda image: tf.compat.v1.image.resize(image, (w, h), method=0))(inputs)
    ...
    if supervision == True:
        ...
        if weight_fusion == True:
            output = WF_block(fusion, 24, normalize=normalize)
            output = Conv2D(classes, (1, 1), activation=activation, name='output_conv_wf',
                            kernel_initializer='he_normal')(output)

        elif ex_supervision == True:
            output5 = conv_bn(block_40, 32, 1, step='ex_conv1', normalize=normalize)
            output5 = Lambda(lambda x: tf.compat.v1.image.resize(x, (w, h), method=0))(output5)
            output6 = conv_bn(block_31, 32, 1, step='ex_conv2', normalize=normalize)
            output6 = Lambda(lambda x: tf.compat.v1.image.resize(x, (w, h), method=0))(output6)
            output7 = conv_bn(block_22, 32, 1, step='ex_conv3', normalize=normalize)
            output7 = Lambda(lambda x: tf.compat.v1.image.resize(x, (w, h), method=0))(output7)
            output8 = conv_bn(block_13, 32, 1, step='ex_conv4', normalize=normalize)
            output8 = Lambda(lambda x: tf.compat.v1.image.resize(x, (w, h), method=0))(output8)
            fusion = concatenate([output5, output6, output7, output8, fusion], axis=3, name='ex_fn')
            output = Conv2D(80, 1, padding='same',
                            name='conv_ex_fn', kernel_initializer='he_normal')(fusion)
            output = Activation('relu')(output)
            if use_ps == True:
                output = SubpixelConv2D(upsampling_factor=2)(output)
                output = Conv2D(classes, (1, 1), activation=activation, name='output_conv2',
            ...
            output = Conv2D(256, 1, padding='same',
                            name='conv_fn2', kernel_initializer='he_normal')(fusion)
            '''    
            output = Conv2D(32,3,padding = 'same',
               name='conv_fn3', kernel_initializer = 'he_normal')(fusion)
            '''
            if normalize == 'frn':
                output = FRN()(output)
                output = TLU()(output)
            else:
                # output = GroupNormalization(name='gn_fn1')(output)
                output = Activation('relu')(output)
            if use_ps == True:
                output = SubpixelConv2D(upsampling_factor=2)(output)
                '''
                branch1 =  Conv2D(4,1,padding = 'same', name='conv_branch1', kernel_initializer = 'he_normal')(output)
                branch1 = Activation('relu')(branch1) 
                branch2 =  Conv2D(4,1,padding = 'same', name='conv_branch2', kernel_initializer = 'he_normal')(output)
                branch2 = Activation('relu')(branch2) 
                branch3 =  Conv2D(4,1,padding = 'same', name='conv_branch3', kernel_initializer = 'he_normal')(output)
                branch3 = Activation('relu')(branch3) 
                branch4 =  Conv2D(4,1,padding = 'same', name='conv_branch4', kernel_initializer = 'he_normal')(output)
                branch4 = Activation('relu')(branch4) 
                branch5 =  Conv2D(4,1,padding = 'same', name='conv_branch5', kernel_initializer = 'he_normal')(output)
                branch5 = Activation('relu')(branch5) 
                output = concatenate([branch1,branch2,branch3,branch4,branch5],axis = 3,name = 'wf_fn')
                '''
                # for 1 lesion
                # output = Conv2D(classes,(1, 1), activation = activation,name='output_conv3', kernel_initializer = 'he_normal')(output)
                # for 4 lesions
...
def resat_unet_plusplusv2(pretrained_weights=None,
                  input_size=(512, 512, 1),
                  classes=3,
                  activation='softmax',
                  supervision=True,
    ...
    if use_ps == True:
        w, h = w // 2, h // 2
        x = DeSubpixelConv2D(downsampling_factor=2)(inputs)
        block_00 = standard_block(x, 64, 3, step='00x', normalize=normalize, res=True)
    else:
        # inputs = conv_bn(inputs,12,1,step = 'input',active = True,normalize = 'gn',group=12)  #调整通道数和Subpixel后一样
        w, h = w // 2, h // 2
        x = Lambda(lambda image: tf.compat.v1.image.resize(image, (w, h), method=0))(inputs)
    ...
    if use_fpn == True:
        ...
        fpn1, fpn2, fpn3, fpn4 = fpnv2(x, normalize=normalize)
        pool_10 = merge_block([fpn1, pool_10], 64, step='fpn1', merge=True, normalize=normalize)  # fpn1
    block_10 = standard_block(pool_10, 64, 3, step='10', normalize=normalize, res=True)
    up_01 = up_block(block_10, 64, (w, h), step='01', normalize=normalize)
    # merge_01 = concatenate([up_01,block_00],axis = 3,name = 'conc_01')
    block_00 = AG_block(block_00, block_10, 64, step='00', normalize=normalize)
    merge_01 = merge_block([up_01, block_00], 64, step='01', normalize=normalize)
    ...
    if use_fpn == True:
        pool_20 = merge_block([fpn2, pool_20], 64, step='fpn2', merge=True, normalize=normalize)  # fpn2
    block_20 = standard_block(pool_20, 128, 3, step='20', normalize=normalize, res=True)
    up_11 = up_block(block_20, 64, (w // 2, h // 2), step='11', normalize=normalize)
    # merge_11 = concatenate([up_11,block_10],axis = 3,name = 'conc_11')
    block_10 = AG_block(block_10, block_20, 64, step='10', normalize=normalize)
    merge_11 = merge_block([up_11, block_10], 64, step='11', normalize=normalize)
    ...
    if use_fpn == True:
        pool_30 = merge_block([fpn3, pool_30], 128, step='fpn3', merge=True, normalize=normalize)  # fpn3
    block_30 = standard_block(pool_30, 256, 3, step='30', normalize=normalize, res=True)
    # block_30 = Dropout(0.05,name='dp_1')(block_30)
    up_21 = up_block(block_30, 128, (w // 4, h // 4), step='21', normalize=normalize)
    # merge_21 = concatenate([up_21,block_20],axis = 3,name = 'conc_21')
    block_20 = AG_block(block_20, block_30, 128, step='20', normalize=normalize)
    ...
    if use_fpn == True:
        pool_40 = merge_block([fpn4, pool_40], 256, step='fpn4', merge=True, normalize=normalize)  # fpn4
    block_40 = standard_block(pool_40, 512, 3, step='40', normalize=normalize, res=True)
    block_40 = Dropout(0.2, name='dp_2')(block_40)
    up_31 = up_block(block_40, 256, (w // 8, h // 8), step='31', normalize=normalize)
    # merge_31 = concatenate([up_31,block_30],axis = 3,name = 'conc_31')
    block_30 = AG_block(block_30, block_40, 256, step='30', normalize=normalize)
    ...
    return model
# 融和层添加通道权重
def resat_unet_plusplusv3(pretrained_weights=None,
                  input_size=(512, 512, 1),
                  classes=3,
                  activation='softmax',
                  supervision=True,
    ...
    if supervision == True:
        ...
        if weight_fusion == True:
            ...
            #fusion1 = PAM_Module(in_channels=256)(fusion)
            fusion = CAM_Module()(fusion)
            #fusion = add([fusion1, fusion2])
            #fusion = concatenate([fusion1, fusion2], axis=3, name='conc_cpa')
            output = Conv2D(256, 1, padding='same',
                            name='conv_fn2', kernel_initializer='he_normal', use_bias = True)(fusion)
    ...
    print(model.summary())
    return model
def resat_unet_plusplusv4(pretrained_weights=None,
                  input_size=(512, 512, 1),
                  classes=3,
                  activation='softmax',
                  supervision=True,
    ...
    if use_ps == True:
        w, h = w // 2, h // 2
        #conv00 = conv_bn(inputs, 64, 3, step='input_conv1', normalize=None, ,active=False)
        x = DeSubpixelConv2D(downsampling_factor=2)(inputs)
        conv01 = conv_bn(x,64,3,step='input_conv',active=False,normalize=None)
        block_00_backbone, block_00 = res_block(conv01, 64, 3, step='00', normalize=normalize, down_sample=False)
    else:
        # inputs = conv_bn(inputs,12,1,step = 'input',active = True,normalize = 'gn',group=12)  #调整通道数和Subpixel后一样
        w, h = w // 2, h // 2
        x = Lambda(lambda image: tf.compat.v1.image.resize(image, (w, h), method=0))(inputs)
    ...
    if supervision == True:
        ...
        if weight_fusion == True:
            ...
            if use_ps == True:
                output = SubpixelConv2D(upsampling_factor=2)(output)
                #output = concatenate([conv00, output], axis=3, name='conc_fn2')
                output = conv_bn(output, 64, 3, step='output_conv1', normalize=None)
                output = SubpixelConv2D(upsampling_factor=2)(output)
                # for 1 lesion
                # output = Conv2D(classes,(1, 1), activation = activation,name='output_conv3', kernel_initializer = 'he_normal')(output)
                # for 4 lesions
...
def resat_unet_plusplusv5(pretrained_weights=None,
                  input_size=(512, 512, 1),
                  classes=3,
                  activation='softmax',
                  supervision=True,
    ...
    if use_ps == True:
        w, h = w // 2, h // 2
        x = DeSubpixelConv2D(downsampling_factor=2)(inputs)
        conv00 = conv_bn(x,64,3,step='input_conv',active=True,normalize=normalize)
        block_00_backbone, block_00 = res_block(conv00, 64, 3, step='00', normalize=normalize, down_sample=False)
    else:
        # inputs = conv_bn(inputs,12,1,step = 'input',active = True,normalize = 'gn',group=12)  #调整通道数和Subpixel后一样
        w, h = w // 2, h // 2
    ...
    up_11 = up_block(block_20, 64, (w // 2, h // 2), step='11', normalize=normalize)
    # merge_11 = concatenate([up_11,block_10],axis = 3,name = 'conc_11')
    block_10 = AG_block(block_10, up_11, 64, step='10', normalize=normalize)
    merge_11 = merge_block([up_11, block_10], 64, step='11', normalize=normalize)
    block_11 = standard_block(merge_11, 64, 3, step='11', normalize=normalize)
    up_02 = up_block(block_11, 64, (w, h), step='02', normalize=normalize)
    # merge_02 = concatenate([up_02,block_01,block_00],axis = 3,name = 'conc_02')
    ...
    up_21 = up_block(block_30, 128, (w // 4, h // 4), step='21', normalize=normalize)
    # merge_21 = concatenate([up_21,block_20],axis = 3,name = 'conc_21')
    block_20 = AG_block(block_20, up_21, 128, step='20', normalize=normalize)
    merge_21 = merge_block([up_21, block_20], 128, step='21', normalize=normalize)
    block_21 = standard_block(merge_21, 128, 3, step='21', normalize=normalize)
    up_12 = up_block(block_21, 64, (w // 2, h // 2), step='12', normalize=normalize)
    # merge_12 = concatenate([up_12,block_11,block_10],axis = 3,name = 'conc_12')
    block_11 = AG_block(block_11, up_12, 64, step='11', normalize=normalize)
    merge_12 = merge_block([up_12, block_11, block_10], 64, step='12', normalize=normalize)
    block_12 = standard_block(merge_12, 64, 3, step='12', normalize=normalize)
    up_03 = up_block(block_12, 64, (w, h), step='03', normalize=normalize)
    # merge_03 = concatenate([up_03,block_02,block_01,block_00],axis = 3,name = 'conc_03')
    ...
    up_31 = up_block(block_40, 256, (w // 8, h // 8), step='31', normalize=normalize)
    # merge_31 = concatenate([up_31,block_30],axis = 3,name = 'conc_31')
    block_30 = AG_block(block_30, up_31, 256, step='30', normalize=normalize)
    merge_31 = merge_block([up_31, block_30], 256, step='31', normalize=normalize)
    block_31 = standard_block(merge_31, 256, 3, step='31', normalize=normalize)
    up_22 = up_block(block_31, 128, (w // 4, h // 4), step='22', normalize=normalize)
    # merge_22 = concatenate([up_22,block_21,block_20],axis = 3,name = 'conc_22')
    block_21 = AG_block(block_21, up_22, 128, step='21', normalize=normalize)
    merge_22 = merge_block([up_22, block_21, block_20], 128, step='22', normalize=normalize)
    block_22 = standard_block(merge_22, 128, 3, step='22', normalize=normalize)
    up_13 = up_block(block_22, 64, (w // 2, h // 2), step='13', normalize=normalize)
    # merge_13 = concatenate([up_13,block_12,block_11,block_10],axis = 3,name = 'conc_13')
    block_12 = AG_block(block_12, up_13, 64, step='12', normalize=normalize)
    merge_13 = merge_block([up_13, block_12, block_11, block_10], 64, step='13', normalize=normalize)
    block_13 = standard_block(merge_13, 64, 3, step='13', normalize=normalize)
    up_04 = up_block(block_13, 64, (w, h), step='04', normalize=normalize)
    # merge_04 = concatenate([up_04,block_03,block_02,block_01,block_00],axis = 3,name = 'conc_04')
...
def unet_plusplus(pretrained_weights=None,
                  input_size=(512, 512, 1),
                  classes=3,
                  activation='softmax',
                  supervision=True,
    ...
    if use_fpn == True:
        ...
        pool_10 = merge_block([fpn1, pool_10], 64, step='fpn1', merge=True, normalize=normalize)  # fpn1
    block_10 = standard_block(pool_10, 64, 3, step='10', normalize=normalize)
    up_01 = mini_up_block(block_10, 32, (w, h), step='01', normalize=normalize)
    # merge_01 = concatenate([up_01,block_00],axis = 3,name = 'conc_01')
    merge_01 = merge_block([up_01, block_00], 64, step='01', normalize=normalize)
    block_01 = standard_block(merge_01, 64, 3, step='01', normalize=normalize)
    ...
    if use_fpn == True:
        pool_20 = merge_block([fpn2, pool_20], 64, step='fpn2', merge=True, normalize=normalize)  # fpn2
    block_20 = standard_block(pool_20, 128, 3, step='20', normalize=normalize)
    up_11 = mini_up_block(block_20, 64, (w // 2, h // 2), step='11', normalize=normalize)
    # merge_11 = concatenate([up_11,block_10],axis = 3,name = 'conc_11')
    merge_11 = merge_block([up_11, block_10], 64, step='11', normalize=normalize)
    block_11 = standard_block(merge_11, 64, 3, step='11', normalize=normalize)
    up_02 = mini_up_block(block_11, 32, (w, h), step='02', normalize=normalize)
    # merge_02 = concatenate([up_02,block_01,block_00],axis = 3,name = 'conc_02')
    ...
    block_30 = standard_block(pool_30, 256, 3, step='30', normalize=normalize)
    # block_30 = Dropout(0.05,name='dp_1')(block_30)
    up_21 = mini_up_block(block_30, 128, (w // 4, h // 4), step='21', normalize=normalize)
    # merge_21 = concatenate([up_21,block_20],axis = 3,name = 'conc_21')
    merge_21 = merge_block([up_21, block_20], 128, step='21', normalize=normalize)
    block_21 = standard_block(merge_21, 128, 3, step='21', normalize=normalize)
    up_12 = mini_up_block(block_21, 64, (w // 2, h // 2), step='12', normalize=normalize)
    # merge_12 = concatenate([up_12,block_11,block_10],axis = 3,name = 'conc_12')
    merge_12 = merge_block([up_12, block_11, block_10], 64, step='12', normalize=normalize)
    block_12 = standard_block(merge_12, 64, 3, step='12', normalize=normalize)
    up_03 = mini_up_block(block_12, 32, (w, h), step='03', normalize=normalize)
    # merge_03 = concatenate([up_03,block_02,block_01,block_00],axis = 3,name = 'conc_03')
    ...
    if supervision == True:
        ...
        if weight_fusion == True:
            ...
            output = Conv2D(192, 1, padding='same',
                            name='conv_fn2', kernel_initializer='he_normal')(fusion)
            '''    
            output = Conv2D(32,3,padding = 'same',
               name='conv_fn3', kernel_initializer = 'he_normal')(fusion)
